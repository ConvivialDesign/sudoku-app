...
function saveGame(state) {
  const payload = { ...state, savedAt: getNowISO() };
...
// Track current screen (helps with polish later)
let currentScreen = "home"; // "home" | "game"
...
function showScreen(which) {
  ...
  // Show Home icon only when in game
...
document.addEventListener("DOMContentLoaded", () => {
  if (hasSavedGame()) {
    const ok = confirm("Resume your last game?");
    if (ok) {
      showScreen("game");

      // IMPORTANT: make sure your board is already rendered before restore
      // If your board is generated on load, call restore after that.
